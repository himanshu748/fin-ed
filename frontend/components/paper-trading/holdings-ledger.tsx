...
import { type PaperHoldingQuotes, paperHoldingKey } from '@/lib/paper-trading/valuation';
...
interface HoldingsLedgerProps {
  ...
  onRefreshQuotes(): Promise<boolean>;
}
...
export function HoldingsLedger({
  ...
  return (
    <section aria-labelledby="holdings-heading" className="min-w-0">
      <div className="flex flex-wrap items-end justify-between gap-3 border-b border-[var(--ledger-rule)] px-1 pb-3">
        <div>
          <h2 id="holdings-heading" className="font-display text-xl font-semibold">
            Holdings
          </h2>
          <p className="mt-1 text-sm text-[var(--muted-ink)]">
            Historical cost with trusted Angel One quotes when available.
          </p>
        </div>
        {holdings.length > 0 ? (
          <button
            ...
            className="min-h-11 min-w-11 rounded-[10px] px-3 font-semibold text-[var(--ledger-blue)] hover:bg-[var(--blue-wash)] focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-[var(--ledger-blue)] disabled:cursor-wait disabled:text-[var(--muted-ink)]"
          >
            {quoteStatus === 'loading' ? 'Refreshing quotes' : 'Refresh live quotes'}
          </button>
        ) : null}
      </div>
        ...
        <>
          <table className="hidden w-full table-fixed border-collapse text-left text-sm md:table [&_td]:px-3 [&_td]:py-4 [&_th]:px-3 [&_th]:py-3">
            <caption className="sr-only">
              Paper holdings with historical cost and trusted current quotes when available.
            </caption>
            <thead>
            ...
            <tbody>
              {holdings.map((holding) => {
                const quote = holdingQuotes[paperHoldingKey(holding)];
                const marketValuePaise = quote ? quote.pricePaise * holding.quantity : null;
                const unrealizedPnlPaise =
                  marketValuePaise === null ? null : marketValuePaise - holding.costBasisPaise;
                return (
                  <tr
                  ...
                  >
                    ...
                    <td className={marketValuePaise === null ? 'text-[var(--risk-brick)]' : ''}>
                      {marketValuePaise === null
                        ? 'Unavailable'
                        : formatPaperCurrency(marketValuePaise)}
                    </td>
                    <td
                      className={
                        unrealizedPnlPaise === null || unrealizedPnlPaise < 0
                          ? 'font-semibold text-[var(--risk-brick)]'
                          : 'font-semibold text-[var(--banknote-green)]'
                      }
                    >
                      {unrealizedPnlPaise === null
                        ? 'Unavailable'
                        : formatSignedPaperCurrency(unrealizedPnlPaise)}
                    </td>
                  </tr>
                );
              })}
            ...
            {holdings.map((holding) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="font-data mt-1 text-xs text-[var(--muted-ink)]">
                    {holding.exchange}
                  </p>
                  <dl className="mt-4 grid grid-cols-[minmax(0,1fr)_auto] gap-x-4 gap-y-3 text-sm">
                    ...
                    <dd className="font-data text-right font-semibold tabular-nums">
                      {marketValuePaise === null
                        ? 'Unavailable'
                        : formatPaperCurrency(marketValuePaise)}
                    </dd>
                    ...
                    <dd
                      className={`font-data text-right font-semibold tabular-nums ${
                        unrealizedPnlPaise === null || unrealizedPnlPaise < 0
                          ? 'text-[var(--risk-brick)]'
